...
async function validateRosterMemberForInvite(
  ...
  const existingInvite = await db
    .prepare(
      'SELECT id FROM invites WHERE roster_member_id = ? AND expires_at > datetime("now")',
    )
    .bind(rosterMemberId)
    .first();
...
export async function createInvite(
  ...
  const { name } = await validateRosterMemberForInvite(
    db,
...
async function loadInviteRelations(
  ...
  const { getMemberById } = await import("./members");
  const rosterMember = await getMemberById(
    db,
    inviteRow.roster_member_id,
    inviteRow.orgId,
  );
...
export async function acceptInvite(
  ...
  const { getMemberByEmailGlobal, upgradeToRegistered } =
  ...
  if (existingMember) {
    ...
    const { getMemberOrganization, addMemberToOrganization } =
      await import("./member-organizations");
    const alreadyInOrg = await getMemberOrganization(
      db,
      existingMember.id,
      String(invite.orgId),
    );
    ...
    for (const v of rosterVoices.results) {
      await db
        .prepare(
          "INSERT OR IGNORE INTO member_voices (member_id, voice_id, is_primary, assigned_by) VALUES (?, ?, ?, ?)",
        )
    ...
    for (const s of rosterSections.results) {
      await db
        .prepare(
          "INSERT OR IGNORE INTO member_sections (member_id, section_id, is_primary, assigned_by) VALUES (?, ?, ?, ?)",
        )
...
export async function getPendingInvites(
  ...
  (Invite & { inviter_name: string | null; inviter_email: string | null })[]
> {
...
export async function revokeInvite(
  ...
  const result = await db
    .prepare(
      `DELETE FROM invites WHERE id = ? AND expires_at > datetime('now')`,
    )
    .bind(inviteId)
    .run();
...
export async function getPendingInviteToken(
  ...
  const result = await db
    .prepare(
      'SELECT token FROM invites WHERE roster_member_id = ? AND expires_at > datetime("now")',
    )
    .bind(rosterMemberId)
    .first<{ token: string }>();
