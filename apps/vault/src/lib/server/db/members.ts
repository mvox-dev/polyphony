...
export async function createMember(
  ...
  await db
    .prepare(
      "INSERT INTO members (id, name, email_id, email_contact, invited_by) VALUES (?, ?, ?, NULL, ?)",
    )
 ...
 */
export async function getMemberByEmailId(
  db: D1Database,
  emailId: string,
  orgId: OrgId,
): Promise<Member | null> {
...
export async function getAllMembers(
  ...
  const membersWithRelations = await Promise.all(
    result.results.map((memberRow) =>
      loadMemberRelations(db, memberRow, orgId),
    ),
  );
...
export async function setPrimaryVoice(
  ...
  await db
    .prepare(
      "UPDATE member_voices SET is_primary = ? WHERE member_id = ? AND voice_id = ?",
    )
...
export async function addMemberSection(
  ...
  const existing = await db
    .prepare(
      "SELECT COUNT(*) as count FROM member_sections WHERE member_id = ?",
    )
    .bind(memberId)
    .first<{ count: number }>();
...
export async function setPrimarySection(
  ...
  await db
    .prepare(
      "UPDATE member_sections SET is_primary = ? WHERE member_id = ? AND section_id = ?",
    )
