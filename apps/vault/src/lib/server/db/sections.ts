...
/** Section with assignment count and child info for management UI */
...
function rowToSectionWithCount(row: SectionWithCountRow): SectionWithCount {
	return {
		...
		childNames: row.child_names ? row.child_names.split(',') : []
	};
}
	...
	const { results } = await db
		.prepare(
			...
			SELECT s.*,
				...
				(SELECT COUNT(*) FROM sections c WHERE c.parent_section_id = s.id) AS child_count,
				(SELECT GROUP_CONCAT(c2.name) FROM sections c2 WHERE c2.parent_section_id = s.id ORDER BY c2.display_order) AS child_names
			FROM sections s
			WHERE s.org_id = ?
...
export async function reassignSection(
	...
	await db.prepare('DELETE FROM member_sections WHERE section_id = ?').bind(sourceId).run();
	...
	// Delete remaining duplicates from source
	await db.prepare('DELETE FROM invite_sections WHERE section_id = ?').bind(sourceId).run();
