...
import { GET as GET_SEASON, PATCH, DELETE } from '../../../routes/api/seasons/[id]/+server';
...
describe('GET /api/seasons', () => {
	...
	it('requires authentication', async () => {
		mockGetAuthenticatedMember.mockRejectedValue(Object.assign(new Error('Authentication required'), { status: 401 }));
		const event = createMockEvent();
...
describe('GET /api/seasons/[id]', () => {
	...
	it('includes events when ?events=true', async () => {
		const mockEvents = [{ id: 'ev-1', title: 'Rehearsal', starts_at: '2026-10-01T19:00:00Z' }];
		mockGetSeason.mockResolvedValue(mockSeason);
...
describe('DELETE /api/seasons/[id]', () => {
	...
	it('deletes season and returns success', async () => {
		...
		expect(response.status).toBe(200);
		expect(mockAssertAdmin).toHaveBeenCalled();
		expect(mockDeleteSeason).toHaveBeenCalledWith(expect.anything(), 'season-1', expect.anything());
		const data = await response.json();
