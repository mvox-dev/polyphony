...
interface MockState {
  ...
  memberSections: Map<
    string,
    Array<{ section_id: string; is_primary: number }>
  >; // key: memberId
...
function createCrossOrgDb(opts: SetupOptions = {}): {
  db: D1Database;
  state: MockState;
} {
  const state: MockState = {
    ...
    memberOrgs: new Map([
      ...
      ...(opts.aliceAlreadyInOrgB
        ? [[`${ALICE_ID}:${ORG_B}`, true] as [string, boolean]]
        : []),
    ]),
    memberRoles: new Map(),
  ...
  if (opts.rosterOrgBRoles?.length) {
    state.memberRoles.set(`${ORG_B_ROSTER_ID}:${ORG_B}`, [
      ...opts.rosterOrgBRoles,
    ]);
  }
    ...
    prepare: (sql: string) => ({
      bind: (...params: unknown[]) => ({
        first: async () => {
          ...
          if (
            sql.includes("FROM invites") &&
            sql.includes("roster_member_id = ?")
          ) {
            return null;
          }
          ...
          if (sql.includes("email_id = ?") && sql.includes("org_id = ?")) {
            ...
            for (const [key, member] of state.members.entries()) {
              if (
                member.email_id === emailId &&
                state.memberOrgs.has(`${key}:${orgId}`)
              ) {
                return { ...member };
              }
            }
          ...
          if (
            sql.includes("FROM member_organizations") &&
          ...
          if (
            ...
            const count = Array.from(state.memberOrgs.keys()).filter((k) =>
              k.startsWith(`${memberId}:`),
            ).length;
            return { count };
          }
          return null;
        ...
        run: async () => {
          ...
          if (
            sql.includes("DELETE FROM invites") &&
            sql.includes("token = ?")
          ) {
          ...
          if (
            sql.includes("DELETE FROM member_roles") &&
            sql.includes("org_id = ?")
          ) {
            const [memberId, orgId] = params as [string, string];
          ...
          if (sql.includes("INSERT") && sql.includes("member_voices")) {
            const [memberId, voiceId, isPrimary] = params as [
              string,
              string,
              number,
            ];
            ...
            if (!existing.some((v) => v.voice_id === voiceId)) {
              state.memberVoices.set(memberId, [
                ...existing,
                { voice_id: voiceId, is_primary: isPrimary ?? 0 },
              ]);
            }
            return { meta: { changes: 1 } };
          }
          ...
          if (
            sql.includes("DELETE FROM member_voices") &&
            sql.includes("member_id = ?")
          ) {
            const [memberId] = params as [string];
          ...
          if (sql.includes("INSERT") && sql.includes("member_sections")) {
            const [memberId, sectionId, isPrimary] = params as [
              string,
              string,
              number,
            ];
            ...
            if (!existing.some((s) => s.section_id === sectionId)) {
              state.memberSections.set(memberId, [
                ...existing,
                { section_id: sectionId, is_primary: isPrimary ?? 0 },
              ]);
            }
            return { meta: { changes: 1 } };
          }
          ...
          if (
            sql.includes("DELETE FROM member_sections") &&
            sql.includes("member_id = ?")
          ) {
            const [memberId] = params as [string];
          ...
          if (
            sql.includes("DELETE FROM members") &&
...
describe("#307 — cross-org invite resolve: AC4 — existing roles preserved", () => {
  it("does not remove or alter existing org-A roles when binding member to org B", async () => {
    const { db, state } = createCrossOrgDb({
      aliceOrgARoles: ["admin", "librarian"],
    });
...
describe("#307 — cross-org invite resolve: GAP6 — roster org-B roles transferred", () => {
  it("transfers org-B roles from roster slot to existing member", async () => {
    ...
    expect(aliceOrgBRoles).toEqual(
      expect.arrayContaining(["conductor", "librarian"]),
    );
  });
  ...
  it("removes org-B roles from roster slot after transfer", async () => {
    ...
    const rosterOrgBRoles =
      state.memberRoles.get(`${ORG_B_ROSTER_ID}:${ORG_B}`) ?? [];
...
describe("#307 — cross-org invite resolve: GAP7 — roster voices transferred", () => {
  it("transfers voices from roster slot to existing member", async () => {
    ...
    expect(aliceVoiceIds).toEqual(
      expect.arrayContaining(["voice_soprano", "voice_mezzo"]),
    );
  });
...
describe("#307 — cross-org invite resolve: GAP8 — roster sections transferred", () => {
  it("transfers sections from roster slot to existing member", async () => {
    ...
    expect(aliceSectionIds).toEqual(
      expect.arrayContaining(["section_s1", "section_full"]),
    );
  });
...
describe("#307 — cross-org invite resolve: GAP9 — roster member row deleted when org-less", () => {
  ...
  it("does NOT delete a roster member row that still belongs to another org", async () => {
    ...
    state.memberOrgs.set(
      `${ORG_B_ROSTER_ID}:${createOrgId("org_gamma")}`,
      true,
    );
