...
function createMockDb() {
  ...
  const memberVoices: Map<string, { voice_id: string; is_primary: number }[]> =
    new Map();
  const memberSections: Map<
    string,
      ...
      bind: (...params: unknown[]) => ({
        run: async () => {
          ...
          if (sql.startsWith("INSERT INTO member_voices")) {
            const [member_id, voice_id, is_primary, assigned_by] =
              params as any[];
            const existing = memberVoices.get(member_id as string) ?? [];
            memberVoices.set(member_id as string, [
              ...existing,
              {
                voice_id: voice_id as string,
                is_primary: is_primary as number,
              },
            ]);
            return { meta: { changes: 1 } };
          }
          ...
          if (sql.startsWith("INSERT INTO member_sections")) {
            const [member_id, section_id, is_primary, assigned_by] =
              params as any[];
        ...
        first: async () => {
          ...
          if (
            ...
            for (const [id, member] of members.entries()) {
              ...
              if (
                (member.name as string).toLowerCase() ===
                  (name as string).toLowerCase() &&
                orgs?.has(org_id as string)
              ) {
                return { id };
              }
            }
        ...
        all: async () => {
          ...
          if (
            sql.includes("FROM member_voices") &&
            sql.includes("JOIN voices")
          ) {
            const [member_id] = params as any[];
            const voiceAssignments =
              memberVoices.get(member_id as string) ?? [];
          ...
          if (
            sql.includes("FROM member_sections") &&
            sql.includes("JOIN sections")
          ) {
            const [member_id] = params as any[];
            const sectionAssignments =
              memberSections.get(member_id as string) ?? [];
