...
async function seedOrgsAndAdmin(db: D1Database): Promise<{ adminId: string }> {
	...
	await db
		...
		.run();
	await db
		.prepare('INSERT INTO member_organizations (member_id, org_id, invited_by, joined_at) VALUES (?, ?, NULL, datetime(\'now\'))')
...
async function seedInvite(
	...
	await db
		...
		.bind(opts.id, opts.orgId, opts.rosterMemberId, opts.token, 'Test Invite', opts.invitedBy, opts.expiresAt)
		.run();
}
...
async function memberExists(db: D1Database, memberId: string): Promise<boolean> {
	const row = await db
...
async function inviteExists(db: D1Database, inviteId: string): Promise<boolean> {
	const row = await db
...
async function getMemberEmail(db: D1Database, memberId: string): Promise<string | null> {
	const row = await db
...
async function getMemberOrgIds(db: D1Database, memberId: string): Promise<string[]> {
...
describe('acceptInvite (integration) — normal path', () => {
	...
	it('returns failure for invalid token without touching DB', async () => {
		const result = await acceptInvite(db, 'tok_does_not_exist', 'any@example.com');
...
describe('acceptInvite (integration) — cross-org path', () => {
	...
	it('links existing member to org B without FK violation (proves #307 delete order)', async () => {
		...
		await db
			.prepare('INSERT INTO members (id, name, email_id, invited_by) VALUES (?, ?, ?, NULL)')
			.bind(aliceId, 'Alice Cross', 'alice@cross.com')
			.run();
		await db
			.prepare('INSERT INTO member_organizations VALUES (?, ?, NULL, NULL, datetime(\'now\'))')
	...
	it('deletes the invite BEFORE the roster member (FK order proof)', async () => {
		...
		await db
			.prepare('INSERT INTO members (id, name, email_id, invited_by) VALUES (?, ?, NULL, NULL)')
