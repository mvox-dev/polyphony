...
export const ssoHandle: Handle = async ({ event, resolve }) => {
  ...
  if (
    event.url.searchParams.get("sso_attempted") === "1" ||
    event.cookies.get("sso_attempted")
  ) {
    return resolve(event);
  }
...
const requestLogHandle: Handle = async ({ event, resolve }) => {
  ...
  console.log(
    `[REQ] ${event.request.method} ${pathname} — ${who} — org:${org?.subdomain ?? "none"}`,
  );
  return resolve(event);
};
...
const localeHandle: Handle = async ({ event, resolve }) => {
  ...
  if (
    existingCookie &&
    (locales as readonly string[]).includes(existingCookie)
  ) {
    return resolve(event);
  }
  ...
  if (
    prefs.language &&
    (locales as readonly string[]).includes(prefs.language)
  ) {
