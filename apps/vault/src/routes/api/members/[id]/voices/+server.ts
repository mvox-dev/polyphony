...
export async function PUT(event: RequestEvent) {
  ...
  if (!parsed.success) {
    throw error(
      400,
      "Invalid request body: voiceIds must be an array of strings",
    );
  }
  ...
  const isAdminOrOwner =
    hasRole(currentMember, "admin", String(orgId)) ||
    hasRole(currentMember, "owner", String(orgId));
  const isConductor = hasRole(currentMember, "conductor", String(orgId));
  const isSectionLeader = hasRole(
    currentMember,
    "section_leader",
    String(orgId),
  );
  ...
  if (!isSelf && !isAdminOrOwner && !isConductor) {
    if (isSectionLeader) {
      ...
      const currentSectionIds = new Set(
        currentMember.sections.map((s) => s.id),
      );
      const hasOverlap = targetMember.sections.some((s) =>
        currentSectionIds.has(s.id),
      );
      if (!hasOverlap) {
        throw error(
          403,
          "Section leader can only edit members in their section",
        );
      }
    } else {
  ...
  return json({
    message: "Voices updated successfully",
