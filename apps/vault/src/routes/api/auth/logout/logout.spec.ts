 ...
 */
function createMockEvent(options: { origin?: string; cookies?: Record<string, string> } = {}) {
 ...
 */
async function getRedirect(event: Parameters<typeof GET>[0]): Promise<{ status: number; location: string }> {
	try {
		await GET(event);
...
describe('GET /api/auth/logout (#301)', () => {
	...
	it('deletes the sso_attempted cookie', async () => {
		...
		const ssoAttemptedDelete = deletedCookies.find((c) => c.name === 'sso_attempted');
